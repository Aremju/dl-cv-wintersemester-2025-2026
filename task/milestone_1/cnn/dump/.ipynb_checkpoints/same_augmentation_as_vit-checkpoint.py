...
def train_model(model_ft, criterion, optimizer, scheduler, num_epochs=50):
    ...
    # --- Save logs as JSON ---
    with open(os.path.join(os.getcwd(), f"train_logs_{timestamp}.json"), "w") as f:
        json.dump(train_logs, f, indent=4)
    with open(os.path.join(os.getcwd(), f"val_logs_{timestamp}.json"), "w") as f:
...
def test_model(model, criterion):
    ...
    with open(os.path.join(os.getcwd(), f"model_report_{timestamp}.json"), "w") as f:
