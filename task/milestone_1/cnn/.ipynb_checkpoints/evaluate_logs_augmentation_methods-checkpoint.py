...
LOG_DIR = os.path.join('logs_augmentierung')
...
def find_scheduler_runs(log_dir):
    """Group files by augmentation key using multiple naming patterns."""
    runs = {}
...
def pretty_label(key):
    """Map internal keys to pretty augmentation names."""
    mapping = {
        'cnn_aug': 'CNN Aug',
        'no_aug': 'No Aug',
        'vit_aug': 'ViT Aug'
...
def plot_lr_scheduler_val_metrics(log_dir, out_dir):
    """Plot validation accuracy and loss for augmentation variants."""
    ...
    out_path = os.path.join(out_dir, "aug_val_metrics.png")
