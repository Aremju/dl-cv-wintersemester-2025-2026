...
def loaddata(data_dir, batch_size, set_name, shuffle):
    ...
    dataloaders = {
        x: DataLoader(image_datasets[x], batch_size=batch_size, shuffle=(x == 'train'), pin_memory=pin_memory)
...
def train_model(model_ft, criterion, optimizer, lr_scheduler, num_epochs=5):
    ...
    for epoch in range(epoch_to_resume_from, num_epochs):
        ...
        for i, (inputs, labels) in enumerate(dset_loaders['train']):
            ...
            if i % 100 == 0:
                print(f"Epoch {epoch+1}/{num_epochs}, Step {i}/{len(dset_loaders['train'])}, Loss: {loss.item():.3f}")

            running_loss += loss.item() * inputs.size(0)
            running_corrects += torch.sum(preds == labels.data)

        epoch_loss = running_loss / dset_sizes['train']
        epoch_acc = running_corrects.double() / dset_sizes['train']
    ...
    model_out_path = os.path.join(save_dir, net_name + '.pth')
...
def test_model(model, criterion):
    ...
    print('Loss: {:.4f} Acc: {:.4f}'.format(running_loss / dset_sizes['test'],
                                            running_corrects.double() / dset_sizes['test']))
